# Story 05 Task 02 Plan: Design Search Indexing Pipeline

## Current State

- Story 05 Task 01 selected Meilisearch because the committed benchmark showed better typo/fuzzy ranking while preserving exact, document text, stored HTML, entity metadata, mixed-query, and update behavior.
...
The crate owns the search boundary and exposes a small pure mapping API that Task 03 can connect to PostgreSQL and Meilisearch:
...
Design a durable indexing state owned by PostgreSQL migrations in Task 03:
...
Task 04 should expose a stable response derived from Meilisearch hits:
...
Update `docs/search-engine-investigation.md` or add `docs/search-indexing-pipeline.md` with a Task 02 storage estimate based on the real Task 01 measurement:

- Task 01 fixture: 5 records used 3,579 bytes in both committed engine result files.
...
- Because the Task 01 fixture is tiny, document the estimate as a sizing model, not a guarantee. Task 05 must replace it with real synced-data measurement.
...
- [x] RED 6: Add a documentation validation test that fails unless `docs/search-indexing-pipeline.md` covers backfill, incremental indexing, retry/failure recovery, update/delete propagation, API result shape, and the storage estimate using Task 01 measurements.
...
- [x] Update acceptance boxes in `task-02-design-search-indexing.md`.
- [x] Set `<passes>true</passes>`.
- [x] Run `/bin/bash .ralph/task_switch.sh`.
- [ ] Add all files, including `.ralph` updates, commit with `task finished task-02-design-search-indexing: ...`, push, and quit immediately.
