# Story 04 Task 02 Plan: Fetch and Classify Document Assets

## Current State

- Task 01 already added the storage schema for `document_asset` and `document_content`.
...
- Boundary correction: `document_asset` owns retrieval attempts, upstream link
  ...
  `document_asset` row in this task; Task 03 can create `document_content` after
...
- Official source preference:
  ...
  - Task 03 will implement extraction, so this task records binary-only selected
...
- [x] Do not let `document_content` become a queue table. If no real text/HTML body exists, store only `document_asset` retrieval state and leave extraction to Task 03.
...
- [x] Update acceptance boxes in `task-02-fetch-and-classify-document-assets.md`.
- [x] Set `<passes>true</passes>`.
- [x] Run `/bin/bash .ralph/task_switch.sh`.
- [ ] Add all files, including `.ralph` updates, commit with `task finished task-02-fetch-and-classify-document-assets: ...`, push, and quit immediately.
