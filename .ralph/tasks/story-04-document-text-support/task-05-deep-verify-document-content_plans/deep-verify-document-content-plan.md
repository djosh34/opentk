# Story 04 Task 05 Plan: Deep Verify Document Content

## Current State

- Story 04 Task 02 added `opentk-sync::document_asset` for controlled HTTP fetching, content-type classification, official text/HTML alternative selection, retry status reporting, source hashes, and selected bodies.
- Story 04 Task 03 added `opentk-sync::document_content` with strict fixture validation for official text, HTML, DOCX, and PDF plus `opentk-db::document_assets::record_document_content_extractions`.
- Story 04 Task 04 added `GET /documents/{source_id}/content`, backed by `opentk-db::read_model::get_document_content`.
...
- `opentk-db::sync_verification::StorageVerification` currently reports only total table/index bytes, `html_asset_bytes = 0`, and binary metadata rows. Task 05 requires separated storage evidence for links/metadata, extracted text, stored HTML, and indexes/constraints.
...
Add one shared test-support pipeline for Story 04 deep verification:
...
Do not run `make test-long` for this normal task unless finishing the entire Story 04 validation gate or unless the final task instructions explicitly require story-end validation at that point. The ignored long-lane test must still be present and selectable by `make test-long`.

After all required checks pass:

- [x] Update acceptance boxes in `task-05-deep-verify-document-content.md`.
- [x] Set `<passes>true</passes>`.
- [x] Run `/bin/bash .ralph/task_switch.sh`.
- [x] Add all files, including `.ralph` updates, commit with `task finished task-05-deep-verify-document-content: ...`, push, and quit immediately.
