...
Story 6 Tasks 1 and 2 already moved application settings into `opentk-config`, renamed `complete-sync` to `opentk-sync`, removed legacy env/CLI setting reads, and added API search degradation during `build_app`. Remaining gaps for this task:
...
- For `opentk-sync`, PostgreSQL and SyncFeed base URL are both required. This task file calls the historical binary `complete-sync`; the actual current binary is `opentk-sync` after Task 2, so tests and implementation should use `opentk-sync`.
...
- [x] Check every acceptance criterion in `task-3-startup-validation.md`.
- [x] Set `<passes>true</passes>` in `task-3-startup-validation.md`.
- Run `/bin/bash .ralph/task_switch.sh`.
- Commit all files, including `.ralph`, with `task finished task-3-startup-validation: ...` and include implementation summary plus `make check`, `make lint`, and `make test` evidence in the commit message.
- Push.
- Quit immediately.
