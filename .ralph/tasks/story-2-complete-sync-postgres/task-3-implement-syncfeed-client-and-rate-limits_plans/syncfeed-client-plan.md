...
- Task file: `.ralph/tasks/story-2-complete-sync-postgres/task-3-implement-syncfeed-client-and-rate-limits.md`
...
- `SyncFeedClient`
  ...
  - Main methods:
    ...
    - `fetch_category_until_resume(category: impl Into<String>, start: CategoryCursor) -> Result<Vec<SyncFeedPage>, SyncFeedClientError>` for testable end-to-end behavior and for task 5 to replace with streaming/orchestration if needed.
...
- Do not write database rows or progress rows here. Task 5 will decide durable transaction semantics.
...
- Do not parse entity payloads beyond extracting raw embedded content XML. Complete entity parsing belongs to task 4.
...
After all checks pass, update `task-3-implement-syncfeed-client-and-rate-limits.md` acceptance boxes and set `<passes>true</passes>`, run `/bin/bash .ralph/task_switch.sh`, commit all files with `task finished task-3-implement-syncfeed-client-and-rate-limits: ...`, push, and quit immediately.
