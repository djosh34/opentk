...
After all checks pass, update acceptance boxes and set `<passes>true</passes>` in `task-01-create-config-crate.md`, run `/bin/bash .ralph/task_switch.sh`, commit all files with `task finished task-01-create-config-crate: ...`, push, and quit immediately.
