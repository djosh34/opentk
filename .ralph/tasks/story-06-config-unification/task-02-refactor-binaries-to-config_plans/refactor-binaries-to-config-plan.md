...
Finish the binary-side config unification that Task 01 partially prepared. All runtime binaries should load one typed `opentk-config` TOML file, build runtime dependencies from that config, log the effective config with secrets redacted, and remove old hardcoded/default/bootstrap behavior from binary crates.
...
- Check every acceptance criterion in `task-02-refactor-binaries-to-config.md`.
- Set `<passes>true</passes>`.
- Run `/bin/bash .ralph/task_switch.sh`.
- Commit all files, including `.ralph`, with `task finished task-02-refactor-binaries-to-config: ...` and include test evidence in the commit message.
- Push.
- Quit immediately.
