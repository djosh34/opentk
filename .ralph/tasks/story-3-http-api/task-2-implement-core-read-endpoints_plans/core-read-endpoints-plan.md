# Story 3 Task 2 Plan: Core Read Endpoints

## Context Read

- Active task: `.ralph/tasks/story-3-http-api/task-2-implement-core-read-endpoints.md`.
- Story 3 Task 1 already added `opentk-api::router(pool)`, `opentk_api::openapi()`, `/health`, `/openapi.json`, shared `ErrorResponse`, and database-backed API integration test style.
...
- If repeated scalar loading makes the generic entity endpoint muddy, keep Task 2 scoped to typed table scalar columns and add a bug/task for repeated scalars instead of silently ignoring an error path.
...
- Add and commit all changed files, including `.ralph`, with message `task finished task-2-implement-core-read-endpoints: ...`.
- Push the commit.
