...
Requirements:
1. Confirm Story 10 Task 1 completed successfully and identify the report file it produced.
...
9. Append dump details and verification results to the `.ralph/reports/full-sync-report-YYYYMMDD-HHMMSS.md` file from Task 1, or create `.ralph/reports/full-sync-dump-YYYYMMDD-HHMMSS.md` that links to the Task 1 report. The report may include dump path, size, checksum, and verification results, but must not embed dump contents.
...
<acceptance_criteria>
- [ ] Story 10 Task 1 completed successfully before the dump was taken
- [ ] Dump was created from the exact fully synced database measured in the full-sync report
- [ ] Dump file is stored outside tracked source paths or under a git-ignored path
- [ ] Dump file is not committed, staged, tracked, or shown as untracked by `git status --short`
- [ ] Dump file path, size, and checksum were recorded
- [ ] Dump command, timestamps, duration, source database identity, source report path, commit SHA, and worktree state were recorded
- [ ] Dump readability was verified with standard PostgreSQL tooling
- [ ] If practical, disposable restore verification was performed and representative row counts matched the full-sync report
- [ ] Report was updated or a linked dump report was created under `.ralph/reports/` without embedding dump contents
- [ ] Dump summary/report update was emailed to the user using the `email-me` skill
