# Story 5 Task 5 Plan: Deep Verify Search Quality

## Current State

- Story 5 selected Meilisearch and already has production search boundaries in `opentk-search`, PostgreSQL-to-index orchestration in `opentk-db::search_sync`, and HTTP exposure in `opentk-api::search`.
...
- The current evaluator proves the scoring model works, but Task 5 needs a stronger verification harness that judges whether the production search stack is good enough on representative synced data, not just whether small fixtures are functional.
- This is the deep verification task for Story 5. Treat it as story-end validation only after the implementation itself is complete and the default checks are green.
...
Because this is the final deep verification task for Story 5, run `make test-long` only after the default checks pass and only if the repository's current task list confirms this task is finishing the entire story or the implementation explicitly changes long/e2e test selection.

After checks pass:

- [ ] Update acceptance boxes in `task-5-deep-verify-search-quality.md`.
- [ ] Set `<passes>true</passes>`.
- [ ] Run `/bin/bash .ralph/task_switch.sh`.
- [ ] Commit all changes, including `.ralph`, with `task finished task-5-deep-verify-search-quality: deep verify search quality` and include test evidence/challenges in the commit body.
- [ ] Push with `git push`.
