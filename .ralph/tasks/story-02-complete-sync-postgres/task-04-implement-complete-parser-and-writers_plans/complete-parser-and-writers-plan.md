...
- Task file: `.ralph/tasks/story-02-complete-sync-postgres/task-04-implement-complete-parser-and-writers.md`
- Story 02 task 01 plan and implementation: `opentk-core::official_schema` is the source-neutral generated official model.
- Story 02 task 02 plan and implementation: `opentk-db::postgres_schema` is the PostgreSQL schema source and migration renderer.
- Story 02 task 03 plan and implementation: `opentk-sync::syncfeed` fetches Atom pages and exposes raw embedded entity XML plus enclosure URLs.
...
- update every completed acceptance criterion in `task-04-implement-complete-parser-and-writers.md`;
- set `<passes>true</passes>`;
- run `/bin/bash .ralph/task_switch.sh`;
- add and commit all changed files, including `.ralph` files, with `task finished task-04-implement-complete-parser-and-writers: ...`;
- push;
- quit immediately.
