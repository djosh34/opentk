# Story 02 Task 02 Plan: Complete PostgreSQL Schema
...
After all checks pass, update `task-02-design-postgres-schema.md` acceptance boxes and set `<passes>true</passes>`, run `/bin/bash .ralph/task_switch.sh`, commit all files with `task finished task-02-design-postgres-schema: ...`, push, and quit immediately.
