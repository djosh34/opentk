...
- Task file: `.ralph/tasks/story-02-complete-sync-postgres/task-06-deep-verify-complete-sync.md`
- Story 02 tasks 01-05 implementation:
...
- [ ] GREEN 6: Query PostgreSQL catalog sizes and download metadata counts. If there is no HTML asset storage in the current schema, report `html_asset_bytes = 0` explicitly and document that Story 02 currently stores linked binary metadata, not fetched binary bodies.
...
This is the Story 02 finishing verification task and explicitly requires live API smoke tests separated into `make test-long`, so `make test-long` is allowed here as the story-end validation gate.
...
- tick completed acceptance criteria in `task-06-deep-verify-complete-sync.md`;
- set `<passes>true</passes>`;
- run `/bin/bash .ralph/task_switch.sh`;
- add and commit all changed files, including `.ralph` files, with `task finished task-06-deep-verify-complete-sync: ...`;
