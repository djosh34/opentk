...
fn github_docker_workflow_caches_cargo_targets_layers_and_final_assembly() {
    ...
    for required in [
        ...
        "COPY --from=dependency-cache-context /workspace/cargo-home/registry /usr/local/cargo/registry",
        "COPY --from=dependency-cache-context /workspace/cargo-home/git /usr/local/cargo/git",
        "cp -a /usr/local/cargo/registry /workspace/cargo-home/registry",
        "cp -a /usr/local/cargo/git /workspace/cargo-home/git",
